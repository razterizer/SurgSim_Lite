                     ...
                     float time)
{
  if (!mh.has_message(time))
  {
    ...
    {
      mh.add_message(time, "Please use the Rinsing and Suction Device to drain the fluids.", t8x::MessageHandlerLevel::Guide);
    }
    if (health_states.artery_tree_leaking_trans())
    {
      mh.add_message(time, "Stop the bleeding!", t8x::MessageHandlerLevel::Warning);
      score -= 50;
    }
    if (health_states.duct_tree_leaking_trans())
    {
      mh.add_message(time, "The Bile Duct is leaking!", t8x::MessageHandlerLevel::Warning);
      score -= 50;
    }
    if (health_states.cut_transient_gallbladder())
    {
      mh.add_message(time, "You cut on the Gallbladder!", t8x::MessageHandlerLevel::Warning);
      health--;
      score -= 100;
    }
    if (health_states.clipped_transient_gallbladder())
    {
      mh.add_message(time, "You clipped the Gallbladder!", t8x::MessageHandlerLevel::Warning);
      health--;
      score -= 100;
    }
    if (health_states.bleeding_critical_trans())
    {
      mh.add_message(time, "Bleeding is critical!", t8x::MessageHandlerLevel::Fatal);
      score -= 300;
    }
    if (health_states.burned_transient_liver())
    {
      mh.add_message(time, "You burned on the Liver!", t8x::MessageHandlerLevel::Fatal);
      health -= 6;
      score -= 300;
    }
    if (health_states.cut_transient_liver())
    {
      mh.add_message(time, "You cut on the Liver!", t8x::MessageHandlerLevel::Fatal);
      health -= 6;
      score -= 300;
    }
    if (health_states.clipped_transient_liver())
    {
      mh.add_message(time, "You clipped the Liver!", t8x::MessageHandlerLevel::Fatal);
      health -= 3;
      score -= 300;
    }
    if (health_states.duct_root_damaged_transient())
    {
      mh.add_message(time, "Duct Root damaged! Unrecoverable injury!", t8x::MessageHandlerLevel::Fatal);
      health = 0;
      score -= 1000;
    }
    if (health_states.leaky_arteria_hepatica_transient())
    {
      mh.add_message(time, "You damaged Arteria Hepatica!", t8x::MessageHandlerLevel::Fatal);
      health = 0;
      score -= 1000;
    }
    if (health_states.leaky_ductus_hepaticus_transient())
    {
      mh.add_message(time, "You damaged Ductus Hepaticus!", t8x::MessageHandlerLevel::Fatal);
      health = 0;
      score -= 1000;
    }
    if (health_states.leaky_arteria_gastroduodenalis_transient())
    {
      mh.add_message(time, "You damaged Arteria Gastroduodenalis!", t8x::MessageHandlerLevel::Fatal);
      health = 0;
      score -= 1000;
    }
    if (health_states.leaky_ductus_choledochus_transient())
    {
      mh.add_message(time, "You damaged Ductus Choledochus!", t8x::MessageHandlerLevel::Fatal);
      health = 0;
      score -= 1000;
    }
    if (health_states.burned_transient_gallbladder())
    {
      mh.add_message(time, "You burned on the Gallbladder!", t8x::MessageHandlerLevel::Warning);
      health--;
      score -= 100;
    }
  }
